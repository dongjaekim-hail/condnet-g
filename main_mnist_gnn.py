...
class condg_exp():
    def __init__(self, num_input = 28**2):
    ...
    def run_exp(self):
        ...
        #    # update policy
        #    update_pol(loss,loss_pol)

            # y_pred, hs =self.mlp(batchdata)
            # hs = np.stack(hs)
            # us = self.gnn(hs)
            #
            # y_t_pred, hss = self.mlp(batchdata, cond_drop = True, u=us)
            #
            # acc = np.sum(y_t_pred == y)/len(batchdata)
