...
class SimpleCNN(nn.Module):
    ...
    #array([   0,    3,   67,  131,  259,  387, 8579, 8835, 9091, 9101])
        idx = 0
        if not cond_drop:
...
def adj(model, bidirect = True, last_layer = True, edge2itself = True):
    if last_layer:
        ...
        num_nodes = (sum([layer.in_channels for layer in model.conv_layers]) + model.conv_layers[-1].out_channels) + sum([layer.in_features for layer in model.fc_layers]) # conv 노드 + fc 노드
    ...
    for i in range(nl):
        if i < len(model.conv_layers) - 1:
            ...
            num_current = model.conv_layers[i].in_channels + model.conv_layers[i].out_channels
...
def main():
    ...
    args.add_argument('--hidden-size', type=int, default=32) # [TODO]: 256 -> 128
