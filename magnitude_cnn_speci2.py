...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x, cond_drop=False, us=None):
        ...
        if not cond_drop:
            ...
            # Conv1
            x = F.relu(self.conv1(x)) * us[:, layer_cumsum[0]:layer_cumsum[1]].view(-1, layer_cumsum[1] - layer_cumsum[0], 1, 1)

            # Conv2
            x = F.relu(self.conv2(x)) * us[:, layer_cumsum[1]:layer_cumsum[2]].view(-1, layer_cumsum[2] - layer_cumsum[1], 1, 1)
            x = self.pool(x)

            # Conv3
            x = F.relu(self.conv3(x)) * us[:, layer_cumsum[2]:layer_cumsum[3]].view(-1, layer_cumsum[3] - layer_cumsum[2], 1, 1)

            # Conv4
            x = F.relu(self.conv4(x)) * us[:, layer_cumsum[3]:layer_cumsum[4]].view(-1, layer_cumsum[4] - layer_cumsum[3], 1, 1)
            x = self.pool(x)

            x = x.view(-1, 128 * 8 * 8)

            # FC1
            x = F.relu(self.fc1(x)) * us[:, layer_cumsum[4]:layer_cumsum[5]]
            x = self.dropout(x)

            # FC2
            x = F.relu(self.fc2(x)) * us[:, layer_cumsum[5]:layer_cumsum[6]]
            x = self.dropout(x)

            # FC3
            x = self.fc3(x) * us[:, layer_cumsum[6]:layer_cumsum[7]]
...
def main():
    ...
    model.load_state_dict(torch.load('out_cnn_runtime_magnitude_weight_tau=0.62024-08-31_22-12-18.pt'))
