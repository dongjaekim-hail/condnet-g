...
class model_condnet(nn.Module):
    ...
    def forward(self, x):
        ...
        u_i = u_i[:, :channels_cumsum[0]:channels_cumsum[1], :, :]
        h = h * u_i
        i += 1
        ...
        u_i = u_i.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, h.size(2), h.size(3)).to(self.device)
        u_i = u_i[:, :channels_cumsum[i]:channels_cumsum[i + 1], :, :]
        h = h * u_i
...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        torch.save(model.state_dict(), './cnn_cond_cifar10_conv' + 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(args.tau) + dt_string +'.pt')
    wandb.finish()
