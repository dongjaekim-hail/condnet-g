...
# 5. 누적 데이터를 저장할 딕셔너리
cumulative_data = {}
scaling_factor = 535040  # cnn is 98306560 mlp is 535040
...
colors = ["blue", "green", "red", "orange", "black", "black"]  # 각 라인의 색 지정
linestyles = {
    "unst_mlp_mnist_lth_real10": "-",  # Unstructured: 실선
    "st_mlp_mnist_lth_real10": ":"   # Structured: 점선
}
...
for i, (run_name, (x_vals, y_vals)) in enumerate(cumulative_data.items()):
    linestyle = linestyles.get(run_name, "-")  # 기본값은 실선 ("-")
    plt.plot(x_vals, y_vals, color=colors[i], linestyle=linestyle, label=legend_labels[run_name])

plt.xlabel("Progress", fontsize=9)
plt.ylabel(r"Cumulative Flops ($10^{8}$)", fontsize=9)
plt.title("")
...
# 부동소수점 문제 방지를 위해 np.round 사용
xticks = np.round(xticks, 2)  # 소수점 둘째 자리까지 반올림
...
# 9. 그래프 출력
plt.savefig("mlp_cum.pdf", format="pdf", bbox_inches = 'tight')