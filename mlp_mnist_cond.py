...
class model_condnet(nn.Module):
    def __init__(self,args):
        ...
        # temp.append(nn.Linear(mlp_hidden[0], mlp_hidden[1]))
    ...
    def forward(self, x):
        ...
        for i in range(len(self.mlp)-1):
            ...
            print(f"Layer {i}: policy shape: {p_i.shape}, h shape: {h.shape}, u shape: {u.shape}, u_i shape: {u_i.shape}")

            policies.append(p_i)
            sample_probs.append(sampling_prob)
            layer_masks.append(u_i)

        p_i = self.policy_net[-1](h)
