...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x, sampling_size=(32, 32), cond_drop=False, us=None, channels=None):
        ...
        if not cond_drop:
            ...
            x = self.conv1(x)
            x = F.relu(x)
            x = x * us[:, channels_cumsum[i]:channels_cumsum[i + 1]]
            hs.append(x)
            # i += 1
...
def main():
    ...
    wandb.init(project="condgnetre",
               ...
               name='cnn_condg_cifar10_s=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(args.tau)
               )
    ...
    for epoch in range(max_epochs):
        ...
        # save model
        torch.save(mlp_model.state_dict(), './cnn_condg_cifar10_cnn_model_'+ 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(args.tau) + dt_string +'.pt')
        torch.save(gnn_policy.state_dict(), './cnn_condg_cifar10_gnn_policy_'+ 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(args.tau) + dt_string +'.pt')
