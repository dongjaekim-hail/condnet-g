...
def main(ITE=0):
    ...
    checkdir(f"{os.getcwd()}/saves/tautest/mnist/")
    torch.save(model,
               f"{os.getcwd()}/saves/tautest/mnist/initial_state_dict_{args.prune_type}.pth.tar")
    ...
    for _ite in range(args.start_iter, ITERATION):
        ...
        for iter_ in pbar:
            ...
            if iter_ % args.valid_freq == 0:
                test_accuracy, test_tau = test(model, test_loader, criterion)

                # Save Weights
                if test_accuracy > best_accuracy:
                    best_accuracy = test_accuracy
                    checkdir(f"{os.getcwd()}/saves/tautest/mnist/")
                    torch.save(model, f"{os.getcwd()}/saves/tautest/mnist/{_ite}_model_{args.prune_type}.pth.tar")

            # Training
            train_loss, train_accuracy, train_tau = train(model, train_loader, optimizer, criterion)
        ...
        torch.save(model.state_dict(), f"{os.getcwd()}/saves/tautest/mnist/{_ite}_model_{args.prune_type}_final.pth.tar")
        ...
        plt.title(f"Loss Vs Accuracy Vs Iterations (mnist,tautest)")
        plt.xlabel("Iterations")
        plt.ylabel("Loss and Accuracy")
        plt.legend()
        plt.grid(color="gray")
        checkdir(f"{os.getcwd()}/plots/lt/tautest/mnist/")
        plt.savefig(
            f"{os.getcwd()}/plots/lt/tautest/mnist/{args.prune_type}_LossVsAccuracy_{comp1}.png",
            dpi=1200)
        plt.close()

        # Dump Plot values
        checkdir(f"{os.getcwd()}/dumps/lt/tautest/mnist/")
        all_loss.dump(f"{os.getcwd()}/dumps/lt/tautest/mnist/{args.prune_type}_all_loss_{comp1}.dat")
        all_accuracy.dump(
            f"{os.getcwd()}/dumps/lt/tautest/mnist/{args.prune_type}_all_accuracy_{comp1}.dat")

        # Dumping mask
        checkdir(f"{os.getcwd()}/dumps/lt/tautest/mnist/")
        with open(f"{os.getcwd()}/dumps/lt/tautest/mnist/{args.prune_type}_mask_{comp1}.pkl",
                  'wb') as fp:
            pickle.dump(mask, fp)
    ...
    checkdir(f"{os.getcwd()}/dumps/lt/tautest/mnist/")
    comp.dump(f"{os.getcwd()}/dumps/lt/tautest/mnist/{args.prune_type}_compression.dat")
    bestacc.dump(f"{os.getcwd()}/dumps/lt/tautest/mnist/{args.prune_type}_bestaccuracy.dat")

    # Plotting
    a = np.arange(args.prune_iterations)
    plt.plot(a, bestacc, c="blue", label="Winning tickets")
    plt.title(f"Test Accuracy vs Unpruned Weights Percentage (mnist,tautest)")
    ...
    plt.grid(color="gray")
    checkdir(f"{os.getcwd()}/plots/lt/tautest/mnist/")
    plt.savefig(f"{os.getcwd()}/plots/lt/tautest/mnist/{args.prune_type}_AccuracyVsWeights.png",
                dpi=1200)
    plt.close()
...
def train(model, train_loader, optimizer, criterion):
    ...
    return epoch_loss, accuracy, tau
