...
class model_dropout(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.mlp) - 1):
            ...
            # random mask that has same shape as h tensor, which has self.tau proportion of ones and otherwise zeros
            mask = torch.bernoulli(torch.full(h.shape, self.tau)).to(self.device)
...
def main():
    ...
    wandb.init(project="condgtest_dk_test",
                ...
                name='runtime_dropout' + '_tau=' + str(args.tau) + '_' + dt_string
                )

    # create model
    model = model_dropout(args)
