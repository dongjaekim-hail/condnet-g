...
def main():
    ...
    args.add_argument('--lambda_s', type=float, default=30)
    args.add_argument('--lambda_v', type=float, default=0.001)
    args.add_argument('--lambda_l2', type=float, default=5e-4)
    args.add_argument('--lambda_pg', type=float, default=1)
    ...
    args.add_argument('--policy-lr', type=float, default=0.05)
    ...
    policy_optimizer = optim.Adam(gnn_policy.parameters(), lr=args.policy_lr, weight_decay=1e-4)
    ...
    for epoch in trange(max_epochs):
        ...
        torch.save(mlp_model.state_dict(),
                   './0.3t0.001out_mlp_model_' + 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(
                       args.tau) + dt_string + '.pt')
        torch.save(gnn_policy.state_dict(),
                   './0.3t0.001out_gnn_policy_' + 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(
                       args.tau) + dt_string + '.pt')
