...
class Gnn(nn.Module):
    ...
    def forward(self, hs, batch_adj): # [TODO] time cost 큰 구간
        ...
        # hs = self.fc2(hs)
        p = F.sigmoid(hs)
        print(f"Expected tau: {self.minprob} ~ {self.maxprob}, p.mean(): {p.mean().item()}")
        # bernoulli sampling
        p = torch.clamp(p, min=self.minprob, max=self.maxprob)
        u = torch.bernoulli(p).to(device)
        print(f"u.mean(): {u.mean().item()}")
...
def main():
    ...
    wandb.init(project="condgtest",
                ...
                name='0.3t0.001out_condg_cnn_cifar10_s=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(args.tau)
                )
    ...
    for epoch in trange(max_epochs):
        ...
        torch.save(mlp_model.state_dict(),
                   './0.3t0.001out_cnn_model_' + 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(
