...
def main(ITE=0):
    ...
    wandb.init(project="condg_cnn", entity='hails', name='st_cnn_cifar10_lth_real', config=args.__dict__)
    ...
    checkdir(f"{os.getcwd()}/saves/stcnnreal/cifar10/")
    torch.save(model,
               f"{os.getcwd()}/saves/stcnnreal/cifar10/initial_state_dict_{args.prune_type}.pth.tar")
    ...
    for _ite in range(args.start_iter, ITERATION):
        ...
        for iter_ in pbar:
            ...
            if iter_ % args.valid_freq == 0:
                ...
                if test_accuracy > best_accuracy:
                    ...
                    checkdir(f"{os.getcwd()}/saves/stcnnreal/cifar10/")
                    torch.save(model, f"{os.getcwd()}/saves/stcnnreal/cifar10/{_ite}_model_{args.prune_type}.pth.tar")
        ...
        torch.save(model.state_dict(), f"{os.getcwd()}/saves/stcnnreal/cifar10/{_ite}_model_{args.prune_type}_final.pth.tar")
        ...
        plt.title(f"Loss Vs Accuracy Vs Iterations (cifar10,stcnnreal)")
        plt.xlabel("Iterations")
        plt.ylabel("Loss and Accuracy")
        plt.legend()
        plt.grid(color="gray")
        checkdir(f"{os.getcwd()}/plots/lt/stcnnreal/cifar10/")
        plt.savefig(
            f"{os.getcwd()}/plots/lt/stcnnreal/cifar10/{args.prune_type}_LossVsAccuracy_{comp1}.png",
            dpi=1200)
        plt.close()

        # Dump Plot values
        checkdir(f"{os.getcwd()}/dumps/lt/stcnnreal/cifar10/")
        all_loss.dump(f"{os.getcwd()}/dumps/lt/stcnnreal/cifar10/{args.prune_type}_all_loss_{comp1}.dat")
        all_accuracy.dump(
            f"{os.getcwd()}/dumps/lt/stcnnreal/cifar10/{args.prune_type}_all_accuracy_{comp1}.dat")

        # Dumping mask
        checkdir(f"{os.getcwd()}/dumps/lt/stcnnreal/cifar10/")
        with open(f"{os.getcwd()}/dumps/lt/stcnnreal/cifar10/{args.prune_type}_mask_{comp1}.pkl",
                  'wb') as fp:
            pickle.dump(mask, fp)
    ...
    checkdir(f"{os.getcwd()}/dumps/lt/stcnnreal/cifar10/")
    comp.dump(f"{os.getcwd()}/dumps/lt/stcnnreal/cifar10/{args.prune_type}_compression.dat")
    bestacc.dump(f"{os.getcwd()}/dumps/lt/stcnnreal/cifar10/{args.prune_type}_bestaccuracy.dat")

    # Plotting
    a = np.arange(args.prune_iterations)
    plt.plot(a, bestacc, c="blue", label="Winning tickets")
    plt.title(f"Test Accuracy vs Unpruned Weights Percentage (cifar10,stcnnreal)")
    ...
    plt.grid(color="gray")
    checkdir(f"{os.getcwd()}/plots/lt/stcnnreal/cifar10/")
    plt.savefig(f"{os.getcwd()}/plots/lt/stcnnreal/cifar10/{args.prune_type}_AccuracyVsWeights.png",
                dpi=1200)
    plt.close()
