...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            wandb.log({'train/batch_cost': c.item(), 'train/batch_acc': acc, 'train/batch_acc_bf': accbf, 'train/batch_pg': PG.item(), 'train/batch_loss': L.item(), 'train/batch_tau': tau_, 'train/batch_Lb': Lb_, 'train/batch_Le': Le_, 'train/batch_Lv': Lv_})
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(test_loader, 0):
                ...
                masks_.append(us.flatten().detach().cpu().numpy())
            ...
            # 가장 긴 길이 확인
            max_length = max(mask.shape[0] for mask in masks_)

            # 모든 마스크를 동일한 길이로 패딩
            masks_padded = [np.pad(mask, (0, max_length - mask.shape[0])) if mask.shape[0] < max_length else mask for
                            mask in masks_]

            # numpy 배열로 변환
            masks_flattened = np.array(masks_padded)

            # 분산 계산 및 출력
            variance = np.var(masks_flattened)
            print("Variance of the masks:", variance)

            # 마스크 순서 섞기
            # np.random.shuffle(masks_flattened)

            n = len(masks_)  # 또는 n = masks_.size
            indx = np.random.permutation(np.linspace(0, n - 1, n).astype(np.int16))
            masks_ = masks_flattened[indx]

            # 셔플된 마스크로 성능 평가
            accuracies = []
            for count, (data, label) in enumerate(test_loader):
                inputs = data.view(-1, num_inputs).to(device)
                current_batch_size = inputs.size(0)
                # count 인덱스를 사용하여 셔플된 마스크의 각 배치를 선택
                shuffled_mask = torch.tensor(masks_[count]).to(device)

                # 마지막 배치일 경우 원래 크기로 잘라서 사용
                if count == len(test_loader) - 1:
                    # 패딩 전 크기로 자르기
                    shuffled_mask = shuffled_mask[:45472]

                shuffled_mask = shuffled_mask.view(current_batch_size, -1, 1)
                spoutputs, _ = mlp_model(inputs, cond_drop=True, us=shuffled_mask)

                # 정확도 계산
                sppred = torch.argmax(spoutputs.to('cpu'), dim=1)
                spacc = torch.sum(sppred == torch.tensor(label.reshape(-1))).item() / label.shape[0]
                accuracies.append(spacc)

            # 평균 정확도 (낮을수록 좋음)
            avg_accuracy = np.mean(accuracies)
            print("Average Accuracy with Shuffled Masks (lower is better for specificity):", avg_accuracy)
        ...
        # save model
        torch.save(mlp_model.state_dict(), './big_mlp_model_'+ 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(args.tau) + '_lr=' + str(args.learning_rate) + dt_string +'.pt')
        torch.save(gnn_policy.state_dict(), './big_gnn_policy_'+ 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(args.tau) + '_lr=' + str(args.learning_rate) + dt_string +'.pt')
