...
def main():
    ...
    args.add_argument('--tau', type=float, default=0.1)
    args.add_argument('--max_epochs', type=int, default=40)
    args.add_argument('--condnet_min_prob', type=float, default=0.05)
    args.add_argument('--condnet_max_prob', type=float, default=0.95)
    ...
    for epoch in range(max_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            # Lb_ = torch.pow(torch.norm(p.squeeze().mean(axis=0) - torch.tensor(tau).to(device), p=2), 2)
            Lb_ = torch.pow(p.squeeze().mean(axis=0) - torch.tensor(tau).to(device), 2).sum()
            # Le_ = Lb_
