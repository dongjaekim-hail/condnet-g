...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        activation_magnitudes = torch.norm(x, dim=(2, 3))
        sorted_activation_magnitudes, _ = torch.sort(activation_magnitudes, dim=1, descending=True)
        threshold = sorted_activation_magnitudes[:, round(sorted_activation_magnitudes.size(1) * self.tau)]
        mask = activation_magnitudes >= threshold.view(-1, 1)
...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        torch.save(model.state_dict(), './out_cnn_runtime_activation_magnitude' + '_tau=' + str(args.tau) + dt_string + '.pt')
