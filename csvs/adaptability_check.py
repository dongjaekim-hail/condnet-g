...
def main(args):
	...
	for tau in [0.6, 0.9]:
		...
		condnet_tau_df = pd.DataFrame(columns=['layer1', 'layer2', 'layer3'])
		condgnet_tau_df = pd.DataFrame(columns=['layer1', 'layer2', 'layer3'])
		...
		for i in range(len(condnet_list)):
			...
			with torch.no_grad():
				...
				for i, data in enumerate(test_loader, 0):
					...
					uss = [p[:, 784:1296].squeeze(), p[:, 1296:1552].squeeze(), p[:, 1552:].squeeze()]

					taus_condg = [p[:, 784:1296].mean().item(), p[:, 1296:1552].mean().item(), p[:, 1552:].mean().item()]
					taus_cond = [l.mean().item() for l in sample_probs]

					for i in range(3):
						condnet_batch[i].append(sample_probs[i].numpy())
						condgnet_batch[i].append(uss[i].numpy())

						condnet_tau_batch[i].append(taus_cond[i])
						condgnet_tau_batch[i].append(taus_condg[i])

					# save each of condnet_batch to npy
					# save each of condgnet_batch to npy
				# for each in condnet_batch[i], concatenate them all
				condnet_batch_ = [np.concatenate(L, axis=0) for L in condnet_batch]
				# condnet_batch_ = [np.stack(L,axis=0) for L in condnet_batch]
				condnet_batch_ = [L.reshape(-1, L.shape[-1]) for L in condnet_batch_]
				# condnet_batch_ = [np.round(L) for L in condnet_batch_]


				condgnet_batch_ = [np.concatenate(L, axis=0) for L in condgnet_batch]
				# round to 0 or 1

				# condgnet_batch_ = [np.stack(L,axis=0) for L in condgnet_batch]
				condgnet_batch_ = [L.reshape(-1, L.shape[-1]) for L in condgnet_batch_]

				# calculate entropy of each layer
				condnet_entropy = [np.mean(-np.sum(condnet_batch_[i] * np.log(condnet_batch_[i] + 1e-8), axis=0)) for i in range(3)]
				condgnet_entropy = [np.mean(-np.sum(condgnet_batch_[i] * np.log(condgnet_batch_[i] + 1e-8), axis=0)) for i in range(3)]

				condnet_tau_batch_ = [np.stack(L,axis=0) for L in condnet_tau_batch]
				condgnet_tau_batch_ = [np.stack(L,axis=0) for L in condgnet_tau_batch]

				# CALCULATE AVERAGE OF EACH
				condnet_tau_batch_ = [np.mean(L) for L in condnet_tau_batch_]
				condgnet_tau_batch_ = [np.mean(L) for L in condgnet_tau_batch_]

				# append to dataframe
				condnet_tau_df.loc[len(condnet_tau_df)] = condnet_tau_batch_
				condgnet_tau_df.loc[len(condgnet_tau_df)] = condgnet_tau_batch_

				condnet_stability_df.loc[len(condnet_stability_df)] = condnet_entropy
				condgnet_stability_df.loc[len(condgnet_stability_df)] = condgnet_entropy

				print('')

		#save dataframe to csv
		condnet_tau_df.to_csv('./condnet_tau{}.csv'.format(tau))
		condgnet_tau_df.to_csv('./condgnet_tau{}.csv'.format(tau))

		condnet_stability_df.to_csv('./condnet_stability{}.csv'.format(tau))
		condgnet_stability_df.to_csv('./condgnet_stability{}.csv'.format(tau))
