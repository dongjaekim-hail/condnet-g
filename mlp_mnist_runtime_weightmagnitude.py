...
class model_magnitude(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.mlp) - 1):
            weight_mean = torch.mean(self.mlp[i].weight.abs(), dim=1)
        ...
        weight_mean = torch.mean(self.mlp[-1].weight.abs(), dim=1)
        sorted_weight_mean, _ = torch.sort(weight_mean, descending=True)
        mask = weight_mean >= sorted_weight_mean[round(len(sorted_weight_mean) * self.tau)]
        h = self.mlp[-1](h)*mask
        # softmax
...
def main():
    ...
    wandb.init(project="condgtest",
                ...
                name='magnitude_runtime_comparison_125' + '_tau=' + str(args.tau)
                )
