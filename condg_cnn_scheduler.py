...
def main():
    ...
    args.add_argument('--lr_mlp', type=float, default=0.0003)
    ...
    args.add_argument('--multi', type=float, default=0.999)
    ...
    wandb.init(project="condg_cnn",
                config=args.__dict__,
                name='condg_cnn_schedule_s=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(args.tau) + '_ti-'
                )

    C = nn.CrossEntropyLoss()
    # mlp_optimizer = optim.SGD(mlp_model.parameters(), lr=learning_rate,
    #                           momentum=0.9, weight_decay=lambda_l2)
    mlp_optimizer = optim.Adam(mlp_model.parameters(), lr=args.lr_mlp, weight_decay=1e-4)
    ...
    # lr scheduler for policy optimizer
    def lr_lambda(epoch):
    ...
    for epoch in trange(max_epochs):
        ...
        for i, data in enumerate(tqdm(train_loader, 0)):
            ...
            wandb.log({'train/batch_cost': c.item(), 'train/batch_acc': acc, 'train/batch_acc_bf': accbf,
                       ...
                       'train/batch_Lb': Lb_, 'train/batch_Le': Le_, 'train/batch_Lv': Lv_, 'train/lr_policy': policy_optimizer.param_groups[0]['lr']})
        ...
        torch.save(mlp_model.state_dict(),
                   './schedule_cnn_model_' + 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(
                       args.tau) + dt_string + '.pt')
        torch.save(gnn_policy.state_dict(),
                   './schedule_gnn_policy_' + 's=' + str(args.lambda_s) + '_v=' + str(args.lambda_v) + '_tau=' + str(
                       args.tau) + dt_string + '.pt')
